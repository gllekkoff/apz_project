...
Streaming/Cassandra endpoints:
    GET /api/trades                 — trade lookup over hot Cassandra storage
    GET /api/momentum/{symbol}      — recent per-minute market momentum
    GET /api/alerts/whale/{symbol}  — recent whale alerts
    GET /api/alerts/volatility/{symbol}

Batch/Postgres endpoints:
    GET /api/reports/hourly          — pre-computed hourly aggregates
    GET /api/analytics/trading-patterns
    GET /api/analytics/whale-impact  — live computation over Postgres
    GET /api/price/{symbol}          — OHLCV from raw trades
"""
...
def pg_conn():
    """Per-request Postgres connection. psycopg2 is not thread-safe across cursors,
    and FastAPI runs sync endpoints in a threadpool, so one connection per call is
    the safest pattern at this scale."""
