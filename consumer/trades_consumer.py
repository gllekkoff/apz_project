"""trades-raw Kafka topic -> Postgres `trades` table.

Batches inserts to keep write throughput high. `ON CONFLICT (trade_id) DO NOTHING`
makes the consumer idempotent — duplicate deliveries from Kafka at-least-once
semantics are silently ignored.
"""
