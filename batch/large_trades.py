"""Large Trades / Whale Impact Analysis.

For trades above the per-symbol 90th percentile by size within a configurable
lookback period, compute price impact = (avg_price_after - avg_price_before)
/ avg_price_before * 100, where the two windows are 5 minutes flanking the
large trade. Returns aggregate stats per symbol.

This module is callable both from the scheduler (logs results) and from the
REST API (returns the rows so an HTTP handler can serialize them).
"""
...
def compute(period: str = "24 hours", symbol: Optional[str] = None) -> list[dict]:
    """Return whale-impact rows for the given period; optionally filter to one symbol."""
    conn = pg_connect()
    try:
