...
For each symbol and each closed hour in the last 12 hours (the current hour is
excluded because it isn't yet finished), compute:
    trade_count, total_volume_usd, min/max/avg/std price, buy_volume,
    sell_volume, dominant_side.

Idempotent: writes via UPSERT on (symbol, hour_ts). Re-running the job over the
same window overwrites stats for any hour whose underlying trades changed (e.g.
late-arriving trades from Kafka backlog).
"""
