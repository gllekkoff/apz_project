...
For each symbol compute, per hour-of-day (0..23) across ALL accumulated history:
    - avg_trade_count   (mean trades per occurrence of that hour)
    - avg_volume_usd
    - avg_volatility    (mean of intra-hour stddev_samp(price))
    - avg_spread        (mean ask-bid from the quotes table)
    - sample_hours      (how many distinct hour buckets contributed)

Idempotent: UPSERT on (symbol, hour_of_day).
"""
...
def run() -> int:
    ...
    try:
        with conn.cursor() as cur:
            ...
            trade_rows = cur.fetchall()  # (symbol, hod, avg_cnt, avg_vol, avg_vol_std, samples)
